class Graph:
    ...
    def dijkstra(self, start_node):
        shortest_paths = {node: float('infinity') for node in
                          self.nodes}  # dictionary with all nodes with distance infinity
        ...
        while unvisited_nodes:  # loop continues till there are unvisited nodes
            current_node = min(unvisited_nodes, key=lambda node: shortest_paths[
                node])  # node with the smallest tentative distance in unvisited_nodes
            unvisited_nodes.remove(current_node)  # remove current_node from unvisited_nodes

            #############################################################################################

            for neighbor, weight in self.adjacency_list[current_node].items():
                # this loop iterates over all unvisited neighbors of current node
                if neighbor in unvisited_nodes:
                    ...
                    if new_path < shortest_paths[neighbor]:
                        # if new_path is less than the current distance of neighbor from start_node
...
def main():
    ...
    print('adjacency list: ' + str(graph.adjacency_list))
